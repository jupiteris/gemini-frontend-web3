...
class DocumentsList extends Component {
  ...
  onEdit = (item) => {
    this.setState({selectedItem: item, addDocument: true});
  };
  onSave = (data) => {
    this.setState({addDocument: false});
    this.props.dispatch(showLoader());
    this.props.dispatch(createNewDocument(data));
  };
  onUpdate = (body) => {
    this.setState({addDocument: false, selectedItem: {}});
    this.props.dispatch(showLoader());
    this.props.dispatch(updateDocument(body.id, body));
  };
  onDelete = () => {
    this.setState({addDocument: false, selectedItem: {}});
  };
  ...
  componentDidMount() {
    this.props.dispatch(fetchDocumentTypes());
    this.props.dispatch(fetchCompliances());
    this.props.dispatch(fetchDocuments(this.getBodyId()));
  }
  getBodyId = () => {
...
const mapStateToProps = ({ settings, document }) => {
  const { width } = settings;
  const { documents } = document;
  return { width, documents };
};
