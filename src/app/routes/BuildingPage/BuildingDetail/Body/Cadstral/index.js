...
import {
    createNewCadastral,
    fetchCadastralKinds,
    fetchCadastrals, fetchCompliances,
    showLoader,
    updateCadastral
} from "../../../../../../actions";
...
class CadstralList extends Component {
    ...
    onEdit = (item) => {
        this.setState({selectedItem: item, addCadastral: true});
    };
    onSave = (data) => {
        this.setState({addCadastral: false});
        this.props.dispatch(showLoader());
        this.props.dispatch(createNewCadastral(data));
    };
    onUpdate = (body) => {
        this.setState({addCadastral: false, selectedItem: {}});
        this.props.dispatch(showLoader());
        this.props.dispatch(updateCadastral(body.id, body));
    };
    onDelete = () => {
        this.setState({addCadastral: false, selectedItem: {}});
    };
    ...
    onAddCadastral = () => {
        this.setState({addCadastral: true});
    };
    ...
    componentDidMount() {
        this.props.dispatch(fetchCadastrals(this.getBodyId()));
        this.props.dispatch(fetchCadastralKinds());
    ...
    constructor(props) {
        super(props);
        this.state = {
            noContentFoundMessage: "No Cadastral found",
            alertMessage: "",
            showMessage: false,
            selectedSectionId: 1,
            drawerState: false,
    ...
    render() {
        const {searchKey, addCadastral, alertMessage, showMessage, noContentFoundMessage, selectedItem, selectPageNum, numPerPage} = this.state;
        const {cadastrals} = this.props;
        const dataList = cadastrals;
        return (
...
const mapStateToProps = ({ settings, cadastral }) => {
    const {width} = settings;
    const { cadastrals } = cadastral;
    return { width, cadastrals };
};
