...
export const getItem = (id, persist = false) => {
  const item = persist ? window.localStorage.getItem(id) : window.sessionStorage.getItem(id);
  try {
...
export const findItem = (id) => {
  const item = window.localStorage.getItem(id) || window.sessionStorage.getItem(id);
  try {
